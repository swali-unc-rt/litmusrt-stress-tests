...
// These FSMLPs all use GPC6, 1 TPC each
//extern FSMLP fsmlp;
//extern FSMLP smlp;
//extern FSMLP all_smlp;
...
public:
    ...
    lt_t submitRequest(//uint64_t smctrl_tpcs_allowed,
        //cudaStream_t* stream,
        //std::function<void()> gpu_launch_fn);
        GPURequest* req);
...
private:
    ...
    //std::deque<GPURequest*> fq;
    pGPURequest FQ[QSIZEMAX];
    int fqend;
    int fqstart;
    int fqsize;
    //std::list<GPURequest*> sq;
    ...
    MCSLock fsmlp_lock;
    //std::mutex fsmlp_lock;
    //SpinLock fsmlp_lock;

    //void get_fsmlp_lock();
    //void exit_fsmlp_lock();
};